...
def run_simulation(i, betas, parameters, n_datasets, n_covariates, n_samples, save_all_files, log_dir, size, num_model_pairs):
    ...
    try:
        ...
        return [i, distances['euclidean'], distances['manhattan'], distances['cosine'], 
                distances['survival'], distances['privacy'], distances['cindex']]
    except Exception as e:
...
def create_result_plots(results):
    ...
    metrics = ['euclidean', 'manhattan', 'cosine', 'survival', 'cindex']
...
def main():
    ...
    results = pd.DataFrame(
        results_list, 
        columns=['run', 'euclidean', 'manhattan', 'cosine', 'survival', 'privacy', 'cindex']
    )
    ...
    # Create result plots
    result_plots = create_result_plots(results)
